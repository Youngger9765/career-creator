...
def upgrade() -> None:
    ...
    op.drop_constraint('counselor_client_relationships_counselor_id_fkey', 'counselor_client_relationships', type_='foreignkey')
    op.drop_constraint('consultation_records_counselor_id_fkey', 'consultation_records', type_='foreignkey')
    
    # Change counselor_client_relationships.counselor_id from UUID to VARCHAR
    op.alter_column('counselor_client_relationships', 'counselor_id',
               existing_type=sa.UUID(),
               type_=sa.String(length=255),
               existing_nullable=False)
    
    # Change consultation_records.counselor_id from UUID to VARCHAR  
    op.alter_column('consultation_records', 'counselor_id',
               existing_type=sa.UUID(),
               type_=sa.String(length=255),
               existing_nullable=False)
