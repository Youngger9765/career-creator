...
def create_card_event(
    event_data: CardEventCreate, session: Session = Depends(get_session)
):
...
def get_room_events(
    ...
    query = (
        query.order_by(CardEvent.created_at, CardEvent.id).offset(offset).limit(limit)
    )
...
def get_room_event_summary(room_id: UUID, session: Session = Depends(get_session)):
    ...
    for event_type in CardEventType:
        count = session.exec(
            select(CardEvent).where(
                and_(CardEvent.room_id == room_id, CardEvent.event_type == event_type)
            )
        ).all()
    ...
    performers = session.exec(
        select(
            CardEvent.performer_id, CardEvent.performer_name, CardEvent.performer_type
        )
        .where(CardEvent.room_id == room_id)
        .distinct()
    ).all()
    ...
    for performer_id, performer_name, performer_type in performers:
        if performer_id and performer_id not in seen:
            unique_performers.append(
                {"id": performer_id, "name": performer_name, "type": performer_type}
            )
    ...
    return {
        ...
        "latest_sequence": (
            max([e.sequence_number for e in total_events]) if total_events else 0
        ),
    }


@router.get("/{event_id}", response_model=CardEventResponse)
def get_card_event(event_id: UUID, session: Session = Depends(get_session)):
...
def delete_card_event(event_id: UUID, session: Session = Depends(get_session)):
