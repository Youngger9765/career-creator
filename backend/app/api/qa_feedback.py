...
async def debug_headers() -> dict[str, Any]:
    """Debug endpoint to check sheet header status."""
    sheet_id = os.getenv("QA_SHEET_ID")
    if not sheet_id:
        return {"error": "QA_SHEET_ID not set"}
    ...
    try:
        ...
        for sheet in sheet_metadata.get("sheets", []):
            ...
            sheets_info.append({"title": title, "sheetId": sid})
            if title == "測試":
                test_sheet_id = sid
            elif title == "統計":
                stats_sheet_id = sid
        ...
        result["test_sheet_id"] = test_sheet_id
        result["stats_sheet_id"] = stats_sheet_id

        # Read first row of 測試 tab
        if test_sheet_id is not None:
            test_result = service.spreadsheets().values().get(
                spreadsheetId=sheet_id,
                range="測試!A1:K1"
            ).execute()
            test_first_row = test_result.get("values", [[]])[0] if test_result.get("values") else []
            result["test_first_row"] = test_first_row
            result["test_headers_expected"] = test_headers
            result["test_headers_match"] = test_first_row == test_headers

        # Read first row of 統計 tab
        if stats_sheet_id is not None:
            stats_result = service.spreadsheets().values().get(
                spreadsheetId=sheet_id,
                range="統計!A1:I1"
            ).execute()
            stats_first_row = stats_result.get("values", [[]])[0] if stats_result.get("values") else []
            result["stats_first_row"] = stats_first_row
            result["stats_headers_expected"] = stats_headers
            result["stats_headers_match"] = stats_first_row == stats_headers
