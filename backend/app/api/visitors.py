...
def join_room_as_visitor(
    ...
    statement = select(Room).where(Room.share_code == share_code, Room.is_active == True)
    ...
    if existing_visitor:
        ...
        existing_visitor.last_seen = visitor_data.last_seen if hasattr(visitor_data, 'last_seen') else existing_visitor.last_seen
