...
from app.models.card_event import CardEvent, CardEventCreate, CardEventType
...
class TestCardEventRetrieval:
    ...
    def test_get_room_events(self, client: TestClient, session: Session, test_room: Room):
    ...
    def test_get_room_events_with_filters(self, client: TestClient, session: Session, test_room: Room):
        ...
        response = client.get(f"/api/card-events/room/{test_room.id}?event_type={CardEventType.CARD_DEALT.value}")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2  # Only CARD_DEALT events
        assert all(event["event_type"] == CardEventType.CARD_DEALT.value for event in data)
        
        # Filter by performer
        response = client.get(f"/api/card-events/room/{test_room.id}?performer_id=user1")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_room_event_summary(self, client: TestClient, session: Session, test_room: Room):
        ...
        events = [
            CardEvent(id=uuid4(), room_id=test_room.id, event_type=CardEventType.CARD_DEALT, performer_id="user1", performer_name="User 1", sequence_number=1),
            CardEvent(id=uuid4(), room_id=test_room.id, event_type=CardEventType.CARD_DEALT, performer_id="user1", performer_name="User 1", sequence_number=2),
            CardEvent(id=uuid4(), room_id=test_room.id, event_type=CardEventType.CARD_FLIPPED, performer_id="user2", performer_name="User 2", sequence_number=3),
            CardEvent(id=uuid4(), room_id=test_room.id, event_type=CardEventType.NOTES_ADDED, performer_id="visitor1", performer_name="Visitor 1", performer_type="visitor", sequence_number=4),
        ]
        ...
        assert data["event_counts_by_type"][CardEventType.CARD_SELECTED.value] == 0  # Not used
    ...
    def test_get_specific_card_event(self, client: TestClient, session: Session, test_room: Room):
...
class TestCardEventDeletion:
    ...
    def test_delete_card_event(self, client: TestClient, session: Session, test_room: Room):
