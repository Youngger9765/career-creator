...
from app.models.visitor import Visitor, VisitorCreate
...
class TestVisitorJoinRoom:
    ...
    def test_join_room_success(self, client: TestClient, test_room: Room):
        ...
        visitor_data = {
            ...
            "room_id": str(test_room.id),  # This gets ignored, room determined by share_code
            "session_id": f"session_{int(time.time())}"
        }
        
        response = client.post(
            f"/api/visitors/join-room/{test_room.share_code}",
            json=visitor_data
        )
    ...
    def test_join_room_invalid_share_code(self, client: TestClient):
        ...
        response = client.post(
            "/api/visitors/join-room/INVALID",
            json=visitor_data
        )
    ...
    def test_join_room_rejoin_existing_session(self, client: TestClient, test_room: Room):
...
class TestVisitorManagement:
    ...
    def test_list_room_visitors(self, client: TestClient, session: Session, test_room: Room):
    ...
    def test_visitor_heartbeat(self, client: TestClient, session: Session, test_room: Room):
        ...
        update_data = {
            "last_seen": "2025-09-13T08:30:00"
        }
        
        response = client.put(
            f"/api/visitors/{visitor.id}/heartbeat",
            json=update_data
        )
    ...
    def test_visitor_heartbeat_not_found(self, client: TestClient):
        ...
        response = client.put(
            f"/api/visitors/{fake_visitor_id}/heartbeat",
            json=update_data
        )
    ...
    def test_visitor_leave_room(self, client: TestClient, session: Session, test_room: Room):
