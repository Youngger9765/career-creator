...
export function Card({
  ...
  const categoryInfo = CARD_CATEGORIES[card.category as keyof typeof CARD_CATEGORIES] || 
                      { name: card.category, color: 'bg-gray-500' };
    ...
    >
      ...
      >
        ...
        <div className="card-face front absolute inset-0 rounded-lg bg-white p-3 flex flex-col">
          {/* Category badge */}
          <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium text-white mb-2 self-start ${categoryInfo.color}`}>
            {categoryInfo.name}
          </div>
