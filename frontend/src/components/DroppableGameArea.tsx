...
export function DroppableGameArea({
  children,
  isActive = false,
  selectedGameRule,
}: DroppableGameAreaProps) {
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-4 border-2 border-dashed border-teal-400 rounded-lg bg-teal-100/20 flex items-center justify-center">
            <div className="text-center text-teal-600">
              <div className="text-lg font-medium mb-2">放開以添加卡片到畫布</div>
              <div className="text-sm opacity-75">{selectedGameRule}遊戲區域</div>
            </div>
          </div>
        </div>
      )}
