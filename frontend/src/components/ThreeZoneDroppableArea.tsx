...
export function ThreeZoneDroppableArea({
  children,
  activeZone,
  selectedGameRule,
}: ThreeZoneDroppableAreaProps) {
          ...
          <div key={zone.id} {...zoneProps}>
            ...
            {(droppable.isOver || activeZone === zone.id) && (
              <div className="absolute inset-0 pointer-events-none">
                <div
                  className={`absolute inset-4 top-16 border-2 border-dashed ${zone.borderColor} rounded-lg ${zone.bgColor} flex items-center justify-center`}
                >
                  <div className={`text-center ${zone.color}`}>
                    <div className="text-lg font-medium mb-2">放開以添加到{zone.title}區域</div>
                    <div className="text-sm opacity-75">{selectedGameRule}遊戲</div>
                  </div>
                </div>
              </div>
            )}
