...
export function useCardEvents({ roomId, realtime = true }: UseCardEventsOptions) {
  ...
  const createEvent = useCallback(async (eventData: Omit<CardEventCreate, 'room_id'>) => {
    try {
  ...
  const dealCard = useCallback((cardId: string, position?: { x: number; y: number }, cardData?: any) => {
    return createEvent({
  ...
  const moveCard = useCallback((cardId: string, fromPosition: { x: number; y: number }, toPosition: { x: number; y: number }) => {
    return createEvent({
  ...
  const addNotes = useCallback((cardId: string | undefined, notes: string) => {
    return createEvent({
  ...
  const recordInsight = useCallback((insight: string, cardIds?: string[]) => {
    return createEvent({
  ...
  const getEventsByType = useCallback((eventType: CardEventType) => {
    return events.filter(event => event.event_type === eventType);
  }, [events]);

  const getEventsByCard = useCallback((cardId: string) => {
    return events.filter(event => event.card_id === cardId);
  }, [events]);

  const getEventsByPerformer = useCallback((performerId: string) => {
